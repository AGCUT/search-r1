...
echo "8. 检查项目结构"
echo "---"
if [ -d "ccks2025_pdf_multimodal/round_b" ]; then
    check_pass "round_b 目录存在"

    ROUND_B_FILES=("b_train_test_preprocess.py" "test_b_style_refer_215.py" "gme_inference.py")
    for file in "${ROUND_B_FILES[@]}"; do
        if [ -f "ccks2025_pdf_multimodal/round_b/$file" ]; then
            check_pass "  $file"
        else
            check_fail "  $file (不存在)"
        fi
...
if [ "$FAILED_CHECKS" -eq 0 ]; then
    ...
    echo "  - 下载模型: 参考 docs/REPRODUCTION_GUIDE.md 步骤0.3"
    echo "  - 准备数据: 参考 docs/REPRODUCTION_GUIDE.md 步骤0.4"
    exit 1
fi