# QReCC数据集实施计划

## 📋 项目概览

### 项目目标
将Search-R1从**单轮问答**扩展到**对话式检索问答**，让模型学会：
1. 理解对话上下文
2. 根据对话历史改写检索query
3. 在多轮对话中进行推理和搜索

### 核心创新点
- **对话上下文建模**：不是独立回答问题，而是理解之前的问答历史
- **Query改写能力**：将省略的指代词补全（如"他是谁" → "Barack Obama是谁"）
- **渐进式信息收集**：通过多轮对话逐步收集信息

---

## 📊 阶段一：数据集理解与分析

### 1.1 下载并探索QReCC数据集
**任务清单：**
- 从HuggingFace下载QReCC数据集
- 理解数据集结构和字段含义
- 统计数据集规模和特征
- 分析对话长度分布
- 查看几个典型样本

**关键字段分析：**
- `Conversation_no`: 对话ID
- `Turn_no`: 对话轮次
- `Question`: 当前问题（可能有省略/指代）
- `Answer`: 标准答案
- `Conversation_context`: 对话历史
- `Rewrite`: 改写后的独立问题（重要！）
- `Conversation_source`: 数据来源

**代码示例：**
```python
# 下载数据集
datasets.load_dataset('McGill-NLP/QReCC')

# 探索数据
from datasets import load_dataset
dataset = load_dataset('McGill-NLP/QReCC')
print(dataset)
print(dataset['train'][0])  # 查看第一个样本
```

### 1.2 对比Search-R1原始数据格式
**分析差异：**
- 原Search-R1: 单轮独立问答
- QReCC: 多轮对话，需要上下文

**需要解决的问题：**
- 如何表示对话历史？
- 是否使用改写后的query？
- 如何设计prompt模板？

### 1.3 确定技术路线
**两种可能的方案：**

**方案A：端到端对话（推荐）**
- 输入：完整对话历史 + 当前问题
- 模型自己学习改写query
- 更接近真实应用场景

**方案B：使用ground-truth改写**
- 输入：改写后的独立问题
- 降低难度，先验证可行性
- 可作为baseline

**建议：** 先实现方案B验证流程，再实现方案A追求性能

---

## 📐 阶段二：数据处理设计

### 2.1 设计数据Schema
**确定数据结构：**
```python
data = {
    "data_source": "qrecc",
    "prompt": [{
        "role": "user",
        "content": prompt_text,  # 包含对话历史
    }],
    "ability": "conversational-reasoning",
    "reward_model": {
        "style": "rule",
        "ground_truth": {"target": [answer]}
    },
    "extra_info": {
        'conversation_id': conv_id,
        'turn_no': turn_no,
        'rewrite': rewrite_query,
        'index': idx,
    }
}
```

### 2.2 设计Prompt模板
**核心挑战：如何表示对话历史**

**方案B的模板（使用改写后的query）：**
```
Answer the given question.
You must conduct reasoning inside <think> and </think> first every time you get new information.
After reasoning, if you find you lack some knowledge, you can call a search engine by <search> query </search> and it will return the top searched results between <information> and </information>.
You can search as many times as your want.
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations.

Question: {rewrite_query}
```

**方案A的模板（端到端对话）：**
```
Answer the given question based on the conversation history.
You must conduct reasoning inside <think> and </think> first every time you get new information.
After reasoning, if you find you lack some knowledge, you can call a search engine by <search> query </search> and it will return the top searched results between <information> and </information>.
You can search as many times as your want.
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations.

Conversation History:
{format_conversation_history}

Current Question: {current_question}
```

**对话历史格式示例：**
```
Turn 1:
Q: Who is the president of USA?
A: Joe Biden

Turn 2:
Q: How old is he?
```

### 2.3 数据预处理策略
**处理流程：**
1. 按 `Conversation_no` 分组
2. 为每个turn构建上下文（包含之前的所有轮次）
3. 生成训练样本
4. 处理特殊情况（第一轮、超长对话等）

**关键考虑：**
- 对话历史的长度限制（token数）
  - 建议：保留最近3-5轮
  - 或使用滑动窗口
- 历史轮次的表示格式
  - 清晰的Turn标记
- 是否包含历史搜索结果
  - 初期可以不包含，简化问题

**数据划分：**
- 训练集/验证集/测试集划分
- **重要：确保同一对话不跨数据集**
- 可以采样子集进行快速实验（如5k样本）

### 2.4 Corpus准备
**QReCC使用的是Wikipedia作为检索库**

**选项1：使用项目现有的wiki corpus**
- 已下载的 `wiki-18.jsonl`
- 已建立的E5索引

**选项2：使用QReCC官方提供的passage collection**
- 可能格式不同，需要转换
- 可能需要重新建立索引

**建议：** 先使用现有的wiki corpus，保持一致性

---

## 🔧 阶段三：模型与系统适配

### 3.1 修改Prompt生成逻辑
**需要创建的文件：**
- `scripts/data_process/qrecc_search.py`（参考 `nq_search.py`）

**关键函数：**
```python
def make_prefix(dp, template_type, conversation_history=None):
    """
    dp: 当前数据点
    template_type: 'base', 'with_history'
    conversation_history: 对话历史列表
    """
    # 实现prompt生成逻辑
    pass

def format_conversation_history(history, max_turns=5):
    """
    格式化对话历史
    history: [(question, answer), ...]
    max_turns: 保留最近几轮
    """
    pass
```

### 3.2 调整训练配置
**创建新的训练脚本：**
- `train_qrecc_ppo.sh`（基于 `train_ppo.sh`）

**关键配置修改：**
```bash
export DATA_DIR='data/qrecc_search'
export BASE_MODEL='Qwen/Qwen2.5-4B'
export EXPERIMENT_NAME=qrecc-search-r1-ppo-qwen2.5-4b

# Qwen模型需要设置VLLM backend
export VLLM_ATTENTION_BACKEND=XFORMERS

# 需要调整的参数
data.train_files=$DATA_DIR/train.parquet
data.val_files=$DATA_DIR/test.parquet
data.max_prompt_length=6144  # 增加（对话历史更长）
data.max_start_length=3072   # 调整初始长度
max_turns=3                  # 根据QReCC平均对话长度设置
retriever.url="http://127.0.0.1:8000/retrieve"
retriever.topk=3

# 其他参数可先保持默认
```

**参数说明：**
- `max_prompt_length`: 需要容纳对话历史，建议4096-8192
- `max_start_length`: 用户输入的最大长度
- `max_turns`: RL允许的最大搜索轮次
- `max_response_length`: 模型生成的最大长度

### 3.3 Reward函数设计
**基础版本（先实现）：**
- 沿用原项目的EM (Exact Match)
- 与标准答案匹配即给奖励

**实现位置：**
- 查看 `verl/trainer/` 或相关reward计算逻辑
- QReCC的答案可能是列表，需要处理

**增强版本（后续优化）：**
- **Query改写质量奖励**：对比生成的query与ground-truth rewrite
- **检索相关性奖励**：检索到的文档是否包含答案
- **对话连贯性奖励**：是否正确使用了上下文
- **多粒度奖励**：F1, ROUGE, BERTScore等

### 3.4 检索器配置
**选项：**
- 复用现有E5检索器（推荐先用这个）
- 或使用更强的对话检索模型（如ConvDR）

**启动检索服务：**
```bash
conda activate retriever
bash retrieval_launch.sh  # 确认配置正确
```

**验证检索服务：**
```bash
curl -X POST http://127.0.0.1:8000/retrieve \
  -H "Content-Type: application/json" \
  -d '{"query": "who is the president of USA", "topk": 3}'
```

---

## 🧪 阶段四：实验设计

### 4.1 Baseline实验
**目的：验证系统可行性**

**实验1：使用改写后的query（方案B）**
- 直接使用QReCC提供的 `rewrite` 字段
- 跳过模型学习改写的步骤
- 验证数据处理和训练流程正确性
- **预期结果**：模型应该能达到与NQ类似的性能

**实验2：简化对话历史**
- 只保留上一轮对话（Turn-1）
- 降低复杂度
- **预期结果**：验证对话历史的表示是否正确

**实验配置：**
- 小规模数据：5000样本
- 训练轮数：3-5 epochs
- 快速迭代：1-2天完成

### 4.2 逐步增强实验
**实验3：端到端对话（方案A）**
- 使用完整对话历史（不用rewrite字段）
- 让模型自己学习改写
- **难度**：更高
- **预期结果**：性能可能略低于方案B，但更实用

**实验4：不同历史长度**
- 1轮历史 vs 3轮 vs 5轮 vs 全部历史
- 分析历史长度的影响
- 找到最优的历史窗口大小

**实验5：不同prompt模板**
- 对比不同的历史表示方式
  - 格式1：Turn 1: Q/A格式
  - 格式2：User/Assistant格式
  - 格式3：简洁格式（只有QA对）
- A/B测试最佳prompt设计

### 4.3 消融实验
**验证各组件的作用：**

| 实验 | 对话历史 | 改写Query | 检索 | 预期EM |
|------|----------|-----------|------|--------|
| 1    | ❌       | ✅        | ✅   | 基线   |
| 2    | ✅       | ✅        | ✅   | 最高   |
| 3    | ✅       | ❌        | ✅   | 中等   |
| 4    | ✅       | ❌        | ❌   | 最低   |

### 4.4 对比实验
**与现有方法对比：**
- Search-R1在单轮NQ上的表现
- 直接用改写query（方案B） vs 端到端对话（方案A）
- 不同RL算法（PPO vs GRPO）
- 不同模型大小（4B vs 7B）或不同模型系列（Qwen vs Llama）

**对比表格示例：**
| 方法 | EM | F1 | 检索次数 | 推理时间 |
|------|----|----|----------|----------|
| Search-R1 (NQ) | 45.2 | 52.3 | 1.8 | 2.3s |
| Ours (方案B) | ? | ? | ? | ? |
| Ours (方案A) | ? | ? | ? | ? |

---

## 📏 阶段五：评估指标设计

### 5.1 核心指标
**答案质量：**
- **Exact Match (EM)**: 答案完全匹配的比例（主要指标）
- **F1 Score**: Token级别的F1分数
- **ROUGE-L**: 最长公共子序列相似度

**实现位置：**
- 参考QReCC官方评估脚本
- 或使用Search-R1现有的评估代码

### 5.2 检索质量指标
- **Recall@k**: 前k个检索结果中包含答案的比例
- **MRR (Mean Reciprocal Rank)**: 第一个相关文档的倒数排名
- **检索成功率**: 至少检索到一个相关passage的比例

### 5.3 对话理解指标
- **Query改写准确率**: 生成的query与ground-truth rewrite的相似度
  - 使用ROUGE或BERTScore
  - 仅适用于方案A
- **指代消解成功率**: 正确处理指代词的比例（需要人工标注部分样本）

### 5.4 效率指标
- **平均检索次数**: 每个问题平均调用检索的次数
- **推理token数量**: 生成的总token数
- **推理时间**: 端到端响应时间

### 5.5 分组分析
**按对话轮次分析：**
```python
# 示例代码
results_by_turn = {
    'turn_1': {'em': 0.50, 'f1': 0.58},
    'turn_2-3': {'em': 0.42, 'f1': 0.51},
    'turn_4+': {'em': 0.35, 'f1': 0.45},
}
```

**按问题类型分析：**
- 有指代（需要改写） vs 无指代
- 简单问题 vs 复杂问题
- 第一轮 vs 后续轮次

**评估脚本位置：**
- 创建 `scripts/evaluate_qrecc.py`

---

## 🚀 阶段六：训练执行

### 6.1 环境准备
**检查清单：**
- [ ] GPU资源确认（至少4-8张GPU，推荐8张）
- [ ] 环境依赖安装
  ```bash
  conda activate searchr1
  pip list | grep vllm  # 检查版本
  ```
- [ ] 数据存储空间（至少50GB）
- [ ] 检索服务器部署并验证
  ```bash
  # 测试检索API
  curl -X POST http://127.0.0.1:8000/retrieve \
    -H "Content-Type: application/json" \
    -d '{"query": "test query", "topk": 3}'
  ```
- [ ] wandb账号配置
  ```bash
  wandb login
  ```

### 6.2 小规模试运行
**快速验证（1-2天）：**
- 采样1000条数据
- 训练2-3个epoch
- 检查是否有bug
- 验证训练曲线是否合理

**运行命令：**
```bash
# 修改train_qrecc_ppo.sh中的参数
data.train_data_num=1000
data.val_data_num=200
trainer.total_epochs=3

bash train_qrecc_ppo.sh
```

**验证要点：**
- 训练不报错
- Loss正常下降
- 验证集EM有提升
- 日志正常记录到wandb

### 6.3 完整训练
**训练策略：**
- **Base Model选择**：使用 **Qwen/Qwen2.5-4B**（推荐）
  - 优势：中文和英文能力均衡，上下文理解能力强
  - 备选：Llama-3.2-3B（纯英文任务）或 Qwen2.5-7B（更强性能）
- **训练轮数**：参考原项目15 epochs
- **Checkpoint保存**：每100步保存一次
- **验证频率**：每50步验证一次
- **日志记录**：开启wandb详细日志

**运行命令：**
```bash
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
export VLLM_ATTENTION_BACKEND=XFORMERS  # Qwen模型必需
bash train_qrecc_ppo.sh 2>&1 | tee logs/qrecc_train.log
```

**训练时间估算：**
- 4B模型 + 8卡GPU: 约1-2天
- 7B模型 + 8卡GPU: 约3-5天

**监控要点：**
- 每个epoch的平均reward
- 验证集EM趋势
- KL散度（不要太大）
- GPU利用率

### 6.4 超参数调优
**关键超参数：**

| 参数 | 默认值 | 调优范围 | 说明 |
|------|--------|----------|------|
| actor学习率 | 1e-6 | 5e-7 ~ 5e-6 | 太大可能不稳定 |
| critic学习率 | 1e-5 | 5e-6 ~ 5e-5 | 通常比actor大10倍 |
| KL系数 | 0.001 | 0.0001 ~ 0.01 | 控制与原模型的偏离 |
| batch_size | 512 | 256 ~ 1024 | 受GPU内存限制 |
| max_prompt_length | 6144 | 4096 ~ 8192 | 对话历史长度 |
| retriever.topk | 3 | 1 ~ 5 | 检索返回数量 |

**调优策略：**
1. **先用默认参数**：验证流程正确
2. **基于验证集调整**：每次只改一个参数
3. **记录所有实验**：使用wandb或表格记录
4. **可选：自动调优**：使用Optuna或Ray Tune

**调优实验示例：**
```bash
# 实验1：基线
bash train_qrecc_ppo.sh

# 实验2：降低学习率
# 修改 actor_rollout_ref.actor.optim.lr=5e-7
bash train_qrecc_ppo.sh

# 实验3：增加KL系数
# 修改 algorithm.kl_ctrl.kl_coef=0.005
bash train_qrecc_ppo.sh
```

---

## 📊 阶段七：结果分析与优化

### 7.1 定量分析
**对比表格：**

| 方法 | EM | F1 | Recall@3 | 平均检索次数 | 推理时间 |
|------|----|----|----------|--------------|----------|
| ConvDR (论文baseline) | 35.2 | 43.1 | - | - | - |
| Search-R1 (NQ单轮) | 45.2 | 52.3 | 78.5 | 1.8 | 2.3s |
| Ours (改写query) | ? | ? | ? | ? | ? |
| Ours (端到端) | ? | ? | ? | ? | ? |

**不同配置性能对比：**
- 按对话轮次分组
- 按历史长度分组
- 按prompt模板分组

### 7.2 定性分析
**Case Study（挑选10-20个典型样本）：**

**成功案例分析：**
```
Conversation:
Turn 1: Q: Who is the president of USA?
         A: Joe Biden
Turn 2: Q: How old is he?
         Model Output: <think>根据上文，"他"指的是Joe Biden</think>
                      <search>Joe Biden age</search>
                      <information>Joe Biden was born on November 20, 1942...</information>
                      <think>计算年龄...</think>
                      <answer>81 years old</answer>
         Ground Truth: 81 years old
         Status: ✅ 正确
```

**失败案例分析：**
```
Turn 1: Q: What is the capital of France?
         A: Paris
Turn 2: Q: What is its population?
         Model Output: <think>需要查询人口信息</think>
                      <search>population</search>  ❌ query太模糊
                      <information>返回无关内容...</information>
                      <answer>Unknown</answer>
         Ground Truth: 2.1 million
         Status: ❌ 失败
         原因: 指代消解失败，没有理解"its"指的是Paris
```

### 7.3 错误分析
**统计错误类型分布：**

| 错误类型 | 数量 | 占比 | 示例 |
|----------|------|------|------|
| 指代消解失败 | 150 | 35% | "他"、"它"、"那里" |
| 检索query太模糊 | 80 | 19% | "人口"而非"Paris人口" |
| 检索到无关文档 | 60 | 14% | query正确但检索失败 |
| 推理错误 | 50 | 12% | 有正确信息但答案错误 |
| 格式错误 | 40 | 9% | 缺少标签或格式不对 |
| 其他 | 45 | 11% | - |

**详细分析每种错误：**
1. **指代消解失败**
   - 原因：模型没有正确理解对话历史
   - 解决方案：增强prompt、增加历史轮次、加入示例

2. **检索query太模糊**
   - 原因：模型没有学会改写完整query
   - 解决方案：增加query改写的reward、提供改写示例

3. **检索到无关文档**
   - 原因：检索器性能问题
   - 解决方案：更换检索器、增加reranker、调整topk

### 7.4 迭代改进
**根据分析结果优化：**

**优化1：改进Prompt模板**
```python
# 原prompt
"Conversation History:\n{history}\nCurrent Question: {question}"

# 改进prompt（增加指代消解指导）
"""Conversation History:
{history}

Current Question: {question}

Important: If the question contains pronouns (he, she, it, they, there, etc.),
you must first identify what they refer to based on the conversation history,
and use the full entity name in your search query.
"""
```

**优化2：修改Reward函数**
```python
# 基础reward：只看答案正确性
reward = 1.0 if em_match else 0.0

# 改进reward：增加query质量奖励
query_similarity = compute_similarity(generated_query, ground_truth_rewrite)
retrieval_success = check_if_answer_in_retrieved_docs()
reward = 0.6 * em_match + 0.2 * query_similarity + 0.2 * retrieval_success
```

**优化3：改进检索策略**
- 增加reranker（使用 `search_r1/search/rerank_server.py`）
- 调整topk（3 → 5）
- 尝试dense + sparse混合检索

**优化4：数据增强**
- 添加few-shot示例在prompt中
- 合成更多对话数据（用GPT-4）
- 从失败案例中学习

**重新训练并对比：**
```bash
# 记录优化前的结果
baseline_em = 42.5

# 应用优化后重新训练
bash train_qrecc_ppo_v2.sh

# 对比结果
improved_em = 47.8  # +5.3提升
```

---

## 🎯 阶段八：进阶优化（可选）

### 8.1 Query改写增强
**显式改写模块：**
- 让模型先生成改写query（在 `<rewrite>` 标签中）
- 再执行检索
- 可以与ground-truth改写监督学习结合

**实现方式：**
```python
# 修改prompt
"""
If the current question contains references to previous context,
first rewrite it as a standalone question in <rewrite></rewrite>.
Then use the rewritten question to search.

Example:
Turn 1: Q: Who is the CEO of Apple?
Turn 2: Q: How old is he?
         <rewrite>How old is the CEO of Apple?</rewrite>
         <search>CEO of Apple age</search>
"""
```

**Reward设计：**
- 如果生成了 `<rewrite>` 标签，计算与ground-truth的相似度
- 给予额外奖励

### 8.2 检索策略优化
**动态检索：**
- 根据问题复杂度调整topk
- 简单问题topk=1，复杂问题topk=5
- 自适应决定是否需要检索（有些问题基于对话历史就能回答）

**多样性检索：**
- 避免检索到重复信息
- 使用MMR (Maximal Marginal Relevance)

**迭代检索：**
- 第一次检索后，根据结果生成新的query
- 多次检索并融合信息

### 8.3 多任务学习
**联合训练NQ和QReCC：**
- 同时使用两个数据集
- 提升泛化能力
- 在不同任务间迁移能力

**实现方式：**
```bash
# 合并数据集
python scripts/data_process/merge_datasets.py \
  --datasets nq qrecc \
  --output data/multitask/

# 训练
bash train_multitask_ppo.sh
```

### 8.4 模型蒸馏
**用大模型指导小模型：**

**步骤1：训练大模型**
```bash
# 先训练7B模型
export BASE_MODEL='Qwen/Qwen2.5-7B'
export VLLM_ATTENTION_BACKEND=XFORMERS
bash train_qrecc_ppo.sh
```

**步骤2：生成蒸馏数据**
```python
# 用7B模型生成推理过程
# 提取思维链、检索query等
```

**步骤3：蒸馏到小模型**
```bash
# 用4B模型学习7B模型的行为
# 可以用SFT或继续RL
export BASE_MODEL='Qwen/Qwen2.5-4B'
```

### 8.5 添加Reranker
**使用Cross-Encoder重排序：**
```bash
# 启动reranker服务器
conda activate retriever
python search_r1/search/rerank_server.py \
  --model cross-encoder/ms-marco-MiniLM-L-12-v2 \
  --port 8001
```

**修改训练配置：**
```bash
# train_qrecc_ppo.sh
retriever.rerank_url="http://127.0.0.1:8001/rerank"
retriever.rerank_topk=3
```

### 8.6 Few-shot Prompting
**在prompt中添加示例：**
```python
prompt = """
Answer the question based on conversation history.

Example 1:
History:
Turn 1: Q: Who is the CEO of Tesla?
        A: Elon Musk
Current Question: Where was he born?
Output:
<think>Based on the history, "he" refers to Elon Musk</think>
<search>Elon Musk birthplace</search>
<information>Elon Musk was born in Pretoria, South Africa...</information>
<answer>Pretoria, South Africa</answer>

Example 2:
...

Now your turn:
History:
{conversation_history}
Current Question: {question}
"""
```

---

## 📝 阶段九：文档与总结

### 9.1 代码组织
**推荐目录结构：**
```
Search-R1/
├── docs/
│   └── qrecc_implementation_plan.md  # 本文档
├── scripts/
│   ├── data_process/
│   │   ├── nq_search.py              # 原有的
│   │   └── qrecc_search.py           # 新建 - QReCC数据处理
│   └── evaluate/
│       └── evaluate_qrecc.py         # 新建 - QReCC评估脚本
├── configs/
│   └── train_qrecc_ppo.sh            # 新建 - QReCC训练配置
├── data/
│   ├── nq_search/                    # 原有的
│   └── qrecc_search/                 # 新建
│       ├── train.parquet
│       ├── test.parquet
│       └── README.md                 # 数据说明
├── experiments/
│   └── qrecc_results/                # 新建 - 实验结果
│       ├── baseline/
│       ├── with_history/
│       └── analysis.md
└── verl_checkpoints/
    └── qrecc-search-r1-*/            # 训练checkpoint
```

### 9.2 实验记录
**使用wandb记录所有实验：**
- 项目名称：`Search-R1-QReCC`
- 每个实验的命名规范：`qrecc-{model}-{method}-{date}`
- 记录内容：
  - 训练曲线（loss, reward, KL）
  - 验证集性能（EM, F1）
  - 超参数配置
  - 每个checkpoint的性能

**本地实验表格（experiments/qrecc_results/experiments.md）：**
```markdown
| 实验ID | 日期 | 模型 | 方法 | EM | F1 | 备注 |
|--------|------|------|------|----|----|------|
| exp001 | 2025-01-15 | Qwen2.5-4B | 改写query | ? | ? | baseline |
| exp002 | 2025-01-16 | Qwen2.5-4B | 端到端 | ? | ? | 需要调优 |
| exp003 | 2025-01-17 | Qwen2.5-4B | 端到端+优化prompt | ? | ? | 改进 |
| ... | ... | ... | ... | ... | ... | ... |
```

### 9.3 撰写项目README
**创建 `data/qrecc_search/README.md`：**
```markdown
# QReCC Search Dataset for Search-R1

## 数据集介绍
QReCC (Question Rewriting in Conversational Context) 是一个对话式问答数据集...

## 数据处理
使用 `scripts/data_process/qrecc_search.py` 处理原始数据。

处理步骤：
1. 下载原始数据
2. 按对话分组
3. 构建对话历史
4. 生成训练样本

## 数据格式
每条数据包含：
- data_source: "qrecc"
- prompt: 包含对话历史的提示
- ability: "conversational-reasoning"
- reward_model: 答案匹配规则
- extra_info: 对话元信息

## 使用方法
```bash
# 数据处理
python scripts/data_process/qrecc_search.py

# 训练
bash configs/train_qrecc_ppo.sh

# 评估
python scripts/evaluate/evaluate_qrecc.py
```

## 统计信息
- 训练集: ~11k个对话，~50k个turn
- 测试集: ~1k个对话，~5k个turn
- 平均对话长度: 4.5轮
- 平均问题长度: 8.3 tokens
```

### 9.4 撰写实验报告
**创建 `experiments/qrecc_results/report.md`：**

包含内容：
1. **项目背景**
   - 动机：为什么做QReCC
   - 挑战：对话式检索的难点

2. **方法介绍**
   - 数据处理流程
   - 模型架构
   - 训练方法

3. **实验设置**
   - 数据集规模
   - 超参数配置
   - 训练环境

4. **实验结果**
   - 主要结果表格
   - 消融实验
   - 对比实验

5. **分析讨论**
   - 定量分析
   - 定性分析（case study）
   - 错误分析

6. **结论与未来工作**
   - 主要贡献
   - 局限性
   - 未来改进方向

---

## ⚠️ 预期挑战与解决方案

### 挑战1：对话历史过长
**问题：** 完整对话历史可能超过模型最大长度限制（如4096 tokens）

**解决方案：**
- **方案A：截断早期历史**
  - 只保留最近N轮（N=3~5）
  - 实现：滑动窗口

- **方案B：摘要压缩历史**
  - 用模型总结早期对话
  - 实现：用GPT-4或小模型生成摘要

- **方案C：动态选择**
  - 根据重要性选择保留哪些轮次
  - 实现：用相似度筛选相关历史

**代码示例：**
```python
def truncate_history(history, max_turns=5):
    """保留最近max_turns轮"""
    return history[-max_turns:]
```

### 挑战2：训练不稳定
**问题：** RL训练可能不收敛，reward震荡

**解决方案：**
- **降低学习率**：从1e-6降到5e-7
- **增加warmup**：`lr_warmup_steps_ratio=0.3`
- **尝试GRPO**：比PPO更稳定
  ```bash
  bash train_qrecc_grpo.sh  # 使用GRPO算法
  ```
- **先用SFT预训练**：
  - 用少量人工标注数据做监督学习
  - 再用RL微调

### 挑战3：检索效率低
**问题：** 对话式检索需要频繁调用，延迟高

**解决方案：**
- **使用ANN索引**：
  - 从Flat索引切换到IVF或HNSW
  - 参考 `search_r1/search/index_builder.py`

- **批量检索**：
  - 一次性检索多个query
  - 修改检索API支持batch

- **缓存机制**：
  - 缓存热门query的结果
  - 使用Redis或内存缓存

### 挑战4：评估困难
**问题：** 对话质量难以用单一指标量化

**解决方案：**
- **多指标综合评估**：
  - EM（主要）
  - F1、ROUGE（辅助）
  - Query质量（改写相似度）
  - 检索质量（Recall）

- **人工评测**：
  - 随机采样100个样本
  - 人工评估：正确性、连贯性、可解释性

- **与baseline详细对比**：
  - 不仅比总体性能
  - 还要按轮次、问题类型分组对比

### 挑战5：数据处理复杂
**问题：** QReCC的对话结构复杂，处理容易出错

**解决方案：**
- **单元测试**：
  ```python
  def test_process_conversation():
      # 测试对话处理是否正确
      sample = {...}
      result = process_conversation(sample)
      assert result['prompt'][0]['content'].startswith("History:")
  ```

- **可视化验证**：
  ```python
  # 打印处理后的样本
  for i in range(5):
      print(f"=== Sample {i} ===")
      print(processed_data[i])
  ```

- **小规模验证**：
  - 先处理100条数据
  - 手工检查是否正确
  - 再处理完整数据

### 挑战6：Reward设计困难
**问题：** 简单的EM可能不足以引导模型学习query改写

**解决方案：**
- **分阶段reward**：
  - 阶段1：只用EM（验证可行性）
  - 阶段2：加入query改写reward
  - 阶段3：加入检索相关性reward

- **Reward shaping**：
  ```python
  # 基础reward
  reward = 1.0 if em_match else 0.0

  # 中间步骤奖励
  if '<rewrite>' in output:
      rewrite_quality = compute_similarity(...)
      reward += 0.2 * rewrite_quality

  if answer_in_retrieved_docs:
      reward += 0.1
  ```

- **从简单到复杂**：
  - 先用稀疏reward（只看最终答案）
  - 模型有一定能力后，再加入密集reward

---

## 🎓 学习与参考资源

### 必读论文
1. **QReCC原论文**
   - Anantha et al. "Open-Domain Question Answering Goes Conversational via Question Rewriting" (NAACL 2021)
   - 理解数据集设计和baseline方法

2. **Search-R1论文**
   - Jin et al. "Search-R1: Training LLMs to Reason and Leverage Search Engines with RL" (2025)
   - 理解RL训练细节和reward设计

3. **ConvDR (对话式Dense Retrieval)**
   - Yu et al. "ConvDR: Conversational Dense Retrieval" (SIGIR 2021)
   - 对话式检索的SOTA方法

4. **相关工作**
   - DeepSeek-R1: 推理和RL训练
   - RAGEN: RAG的RL训练
   - PPO/GRPO算法原理

### 推荐代码库
1. **原Search-R1代码**
   - `scripts/data_process/nq_search.py` - 数据处理示例
   - `train_ppo.sh` - 训练配置示例
   - `verl/trainer/` - RL训练逻辑

2. **veRL文档**
   - https://verl.readthedocs.io/
   - 多轮对话示例

3. **QReCC官方代码**
   - https://github.com/apple/ml-qrecc
   - 评估脚本和baseline

### 在线资源
- **HuggingFace QReCC页面**: 数据集文档
- **wandb Search-R1项目**: 原项目的训练日志
- **论文解读视频/博客**: 搜索Search-R1和QReCC

---

## ✅ 里程碑检查清单

### 第1周：准备与探索
- [ ] 下载并探索QReCC数据集
- [ ] 阅读QReCC论文和Search-R1论文
- [ ] 理解数据集结构和字段
- [ ] 统计数据集特征（对话长度分布等）
- [ ] 确定技术路线（方案A/B）

### 第2周：数据处理
- [ ] 设计数据schema和prompt模板
- [ ] 编写数据处理脚本 `qrecc_search.py`
- [ ] 处理训练集和测试集
- [ ] 验证处理后的数据（可视化检查）
- [ ] 准备检索corpus（复用或新建）

### 第3周：系统适配
- [ ] 创建训练配置 `train_qrecc_ppo.sh`
- [ ] 调整超参数配置
- [ ] 启动检索服务器并验证
- [ ] 配置wandb日志
- [ ] 小规模试运行（1000样本）

### 第4周：Baseline实验
- [ ] 实验1：使用改写query（方案B）
- [ ] 实验2：简化对话历史
- [ ] 验证训练流程正确性
- [ ] 初步结果分析
- [ ] 调试和修复问题

### 第5-6周：完整训练
- [ ] 完整数据训练（方案B）
- [ ] 监控训练过程
- [ ] 保存最佳checkpoint
- [ ] 在测试集上评估
- [ ] 记录baseline性能

### 第7周：进阶实验
- [ ] 实验3：端到端对话（方案A）
- [ ] 实验4：不同历史长度
- [ ] 实验5：不同prompt模板
- [ ] 对比各实验结果
- [ ] 选择最佳配置

### 第8周：消融与对比
- [ ] 消融实验（有无历史、有无改写等）
- [ ] 与NQ单轮结果对比
- [ ] 与论文baseline对比
- [ ] 不同RL算法对比（PPO vs GRPO）
- [ ] 整理对比表格

### 第9周：深入分析
- [ ] 定量分析（按轮次、按类型）
- [ ] 定性分析（case study）
- [ ] 错误分析（统计错误类型）
- [ ] 识别主要问题
- [ ] 制定优化方案

### 第10周：迭代优化
- [ ] 优化prompt模板
- [ ] 改进reward函数
- [ ] 尝试reranker
- [ ] 重新训练
- [ ] 验证改进效果

### 第11-12周：文档与总结
- [ ] 撰写项目README
- [ ] 整理实验记录
- [ ] 编写实验报告
- [ ] 准备演示demo（可选）
- [ ] 代码清理和注释

---

## 🎯 三种推进策略

### 策略1：快速迭代（2-3周）
**适合：快速验证想法、时间紧张**

**里程碑：**
- Week 1: 数据处理 + 系统适配
- Week 2: Baseline训练（方案B，5k样本）
- Week 3: 评估 + 简单优化

**交付物：**
- 基础代码和数据
- 初步实验结果
- 简短报告

### 策略2：完整实施（4-6周）
**适合：课程项目、毕业设计**

**里程碑：**
- Week 1-2: 数据处理 + Baseline
- Week 3-4: 完整训练 + 进阶实验
- Week 5: 分析优化
- Week 6: 文档总结

**交付物：**
- 完整代码和文档
- 多组对比实验
- 详细分析报告
- 可运行的demo

### 策略3：论文级（8-12周）
**适合：发表论文、深入研究**

**里程碑：**
- Week 1-2: 文献调研 + 数据准备
- Week 3-4: Baseline实验
- Week 5-6: 多组对比实验
- Week 7-8: 创新方法实现
- Week 9-10: 深入分析优化
- Week 11-12: 论文撰写

**交付物：**
- 研究级代码
- 全面的实验
- 创新的方法
- 学术论文草稿

---

## 💡 成功关键要素

### 1. 循序渐进
- 不要一开始就做最复杂的
- 先验证baseline，再逐步增强
- 每个阶段都有明确目标

### 2. 详细记录
- 记录每个实验的配置和结果
- 使用wandb自动记录训练过程
- 保存代码版本（git commit）

### 3. 及时分析
- 不要盲目调参
- 每个实验后都要分析原因
- 基于分析结果调整策略

### 4. 适时求助
- 遇到bug及时查文档/issue
- 关键决策可以咨询导师/同学
- 利用开源社区资源

### 5. 保持灵活
- 如果某个方向行不通，及时调整
- 根据资源限制选择合适的规模
- 不必拘泥于原计划

---

## 📞 问题诊断清单

**如果训练不收敛：**
- [ ] 检查数据处理是否正确（打印几个样本）
- [ ] 降低学习率（减半）
- [ ] 检查reward是否合理（打印几个reward值）
- [ ] 尝试更稳定的算法（GRPO）
- [ ] 减少KL系数

**如果性能不理想：**
- [ ] 检查baseline是否正确（与论文对比）
- [ ] 分析错误类型（是哪里出问题）
- [ ] 检查检索质量（Recall@k是否足够高）
- [ ] 尝试不同prompt模板
- [ ] 增加训练数据或epoch

**如果检索太慢：**
- [ ] 使用ANN索引而非Flat
- [ ] 减少topk
- [ ] 批量检索
- [ ] 检查网络延迟

**如果显存不足：**
- [ ] 减少batch_size
- [ ] 开启gradient checkpointing
- [ ] 开启offload（param/grad/optimizer）
- [ ] 使用更小的模型（使用Qwen2.5-3B而非4B，或使用Qwen2.5-1.5B）
- [ ] 减少max_prompt_length

---

## 🎊 预期成果

完成本项目后，你将获得：

1. **技术能力**
   - 深入理解RAG和对话系统
   - 掌握RL训练LLM的实践经验
   - 熟悉大规模模型训练流程

2. **项目成果**
   - 一个可运行的对话式检索问答系统
   - 完整的实验代码和文档
   - 详细的实验报告和分析

3. **潜在应用**
   - 客服对话系统
   - 学术文献检索助手
   - 个人知识库问答

4. **学术价值**（如果做得好）
   - 可以发表workshop论文
   - 可以作为毕业设计
   - 可以开源贡献社区

---

## 📅 时间规划示例（6周完整实施）

| 周 | 日期 | 主要任务 | 时间投入 | 检查点 |
|----|------|----------|----------|--------|
| 1 | Week 1 | 数据集探索 + 数据处理脚本 | 15-20h | 生成train.parquet |
| 2 | Week 2 | 系统适配 + 小规模试运行 | 15-20h | 训练运行成功 |
| 3 | Week 3 | Baseline完整训练 | 10h人工 + 2天GPU | EM > 40% |
| 4 | Week 4 | 进阶实验（3-5组） | 10h人工 + 3天GPU | 多组结果 |
| 5 | Week 5 | 深入分析 + 优化迭代 | 20h | 性能提升 |
| 6 | Week 6 | 文档撰写 + 整理代码 | 15h | 完整交付 |

**总计：** 约85-100小时人工 + 5-6天GPU时间

---

好了，这就是完整的QReCC实施计划！

**建议你现在做的事：**
1. ⭐ 收藏这个文档
2. 📝 根据自己的时间选择推进策略（快速/完整/论文级）
3. ✅ 从第一个检查清单开始执行
4. 💬 有问题随时问我！

祝你项目顺利！🚀